...
function InnerPagesHeader() {
  ...
  return (
    <header className="fixed top-4 left-1/2 transform -translate-x-1/2 w-[95%] max-w-6xl z-50 bg-white/30 backdrop-blur-md shadow-xl border border-white/20 rounded-xl transition-all duration-300">
      <div className="px-4 py-3 flex justify-between items-center">
        {/* Logo */}
        <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight text-blue-700 drop-shadow-sm hover:drop-shadow-lg transition duration-300">
          <a
            href="/#home"
      ...
      {/* Mobile Dropdown */}
      {isOpen && (
        <div className="md:hidden px-4 pb-2 pt-0 bg-white/40 backdrop-blur-sm shadow-lg border-t border-white/20 rounded-b-xl">
          {navItems.map((item) => (
            <a
              key={item.to}
              href={item.to}
